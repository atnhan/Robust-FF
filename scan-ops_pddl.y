...
%token POSS_PRECONDITION_TOK
%token PARAMETERS_TOK
%token EFFECT_TOK
%token POSS_EFFECT_TOK
%token EQ_TOK
%token AND_TOK
%token NOT_TOK
%token WHEN_TOK
...
action_def_body
|
POSS_PRECONDITION_TOK  adl_goal_description
{ 
  scur_op->poss_preconds = $2; 
}
action_def_body
|
...
action_def_body
|
POSS_EFFECT_TOK  adl_effect
{ 
  scur_op->poss_effects = $2; 
}
action_def_body
;
