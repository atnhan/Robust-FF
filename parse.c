...

{
	...
	for ( po = gloaded_ops; po; po = po->next ) {
		 ...
		 * TUAN (Begin)
		 */
		collect_type_names_in_pl( po->poss_preconds );
		collect_type_names_in_pl( po->poss_effects );
		/*
		 * TUAN (End)
		 */
	...
	for ( po = gloaded_ops; po; po = po->next ) {
		 ...
		 * TUAN (begin)
		 */
		make_either_ty_in_pl( po->poss_preconds );
		make_either_ty_in_pl( po->poss_effects );
		/*
		 * Tuan (end)
		 */
	...
	for ( po = gloaded_ops; po; po = po->next ) {
		 ...
		 * TUAN (begin)
		 */
		normalize_tyl_in_pl( &po->poss_preconds );
		normalize_tyl_in_pl( &po->poss_effects );
		/*
		 * TUAN (end)
		 */
...

{
	...
	for ( i = gloaded_ops; i; i = i->next ) {
		 ...
		 */
		if ( !i->poss_preconds ) {
			i->poss_preconds = new_PlNode( TRU );
		}
		if ( !is_wff( i->poss_preconds ) ) {
			printf("\nop %s has illegal possible precondition", i->name);
			return FALSE;
		}
		if ( !make_effects( &(i->poss_effects) ) ) {
			printf("\nop %s has illegal possible effects", i->name);
			return FALSE;
		}
		/*
		 * TUAN (end)
