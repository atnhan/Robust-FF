...
int gnum_possible_annotations = 0;
...
void create_integer_representation( void )
{
	...
	for ( o = gloaded_ops; o; o = o->next ) {
		 ...
		 * TUAN (begin)
		 */
		tmp->poss_preconds = make_Wff( o->poss_preconds, tmp->num_vars );
		/*
		 * TUAN (end)
		 */
		...
		if ( o->poss_effects ) {
			/* in make_effect, make sure that no one afects equality.
			 */
			nn = o->poss_effects->sons;
			while ( nn &&
					(tmp->poss_effects = make_effect( nn, tmp->num_vars )) == NULL ) {
				nn = nn->next;
			}
			if ( nn ) {
				for ( n = nn->next; n; n = n->next ) {
					if ( (tmp->poss_effects->prev = make_effect( n, tmp->num_vars )) == NULL ) {
						continue;
					}
					tmp->poss_effects->prev->next = tmp->poss_effects;
					tmp->poss_effects = tmp->poss_effects->prev;
				}
			}
		}
...

{
	...
	if ( gcmd_line.display_info == 105 ) {
		...
		for ( i = 0; i < gnum_predicates; i++ ) {
			 ...
			 */
			printf(" is %s, %s",
					gis_poss_added[i] ? "POSS_ADDED" : "NOT POSS_ADDED",
							gis_poss_deleted[i] ? "POSS_DELETED" : "NOT POSS_DELETED");
...

{
	...
	for ( i = 0; i < gnum_predicates; i++ ) {
		 ...
		 * TUAN (begin)
		 */
		gis_poss_added[i] = FALSE;
		gis_poss_deleted[i] = FALSE;
		/*
		 * TUAN (end)
		 */
	}
	...
	for ( i = 0; i < gnum_operators; i++ ) {
		...
		for ( e = goperators[i]->poss_effects; e; e = e->next ) {
			for ( l = e->effects; l; l = l->next ) {
				if ( l->negated ) {
					gis_poss_deleted[l->fact.predicate] = TRUE;
				} else {
					gis_poss_added[l->fact.predicate] = TRUE;
				}
			}
		}
		/*
...

{
	...
	for ( i = 0; i < gnum_predicates; i++ ) {
		if ( !gis_added[i] && !gis_deleted[i]
		                                    ...
		                                    * TUAN (begin)
		                                    */
		                                   && !gis_poss_added[i] && !gis_poss_deleted[i]
		                                   /*
		                                    * TUAN (end)
		                                    */
		                                   	  && garity[i] == 1 ) {
	...
	for ( i = 0; i < gnum_full_initial; i++ ) {
		...
		if ( gis_added[p] || gis_deleted[p]
		                                  ...
		                                  * TUAN (begin)
		                                  */
		                                 || gis_poss_added[p] || gis_poss_deleted[p]
		                                 /*
		                                  * TUAN (end)
		                                  */
		) {
...
void split_domain( void )
{
	...
	for ( i = 0; i < gnum_operators; i++ ) {
		if ( (m = is_dnf( goperators[i]->preconds )) != -1 ) {		/* TUAN: if the preconditions are not in DNF, they are hard*/
			for ( e = goperators[i]->effects; e; e = e->next ) {
				if ( is_dnf( e->conditions ) == -1 ) {				/* TUAN: if one of the conditions are not in DNF, they are hard*/
					break;
				}
			}
			if ( !e ) {
	...
	for ( i = 0; i < gnum_operators; i++ ) {
		...
		case OR:
			...
			for ( ww = w->sons; ww; ww = ww->next ) {
				 ...
				 * TUAN (begin)
				 */
				make_normal_possible_preconditions(&tmp_op, goperators[i]);
				make_normal_possible_effects(&tmp_op, goperators[i]);
				/*
				 * TUAN (end)
				 */
...
void make_normal_possible_preconditions(NormOperator **nop, Operator *op) {
	...
	case AND:
		m = 0;
		for (ww = w->sons; ww; ww = ww->next) m++;
		(*nop)->poss_precond_annotation_id = (int*) calloc(m, sizeof(int));
		(*nop)->poss_preconds = (Fact*) calloc(m, sizeof(Fact));
		(*nop)->num_poss_preconds = 0;
		for (ww = w->sons; ww; ww = ww->next) {
			tmp_ft = &((*nop)->poss_preconds[(*nop)->num_poss_preconds]);
			tmp_ft->predicate = ww->fact->predicate;
			a = garity[tmp_ft->predicate];
			for ( j = 0; j < a; j++ ) {
				tmp_ft->args[j] = ww->fact->args[j];
			}
			(*nop)->poss_precond_annotation_id[(*nop)->num_poss_preconds] = ++gnum_possible_annotations;
			(*nop)->num_poss_preconds++;
		}
		break;

	case ATOM:
		(*nop)->poss_precond_annotation_id = (int*) calloc(1, sizeof(int));
		(*nop)->poss_preconds = ( Fact * ) calloc( 1, sizeof( Fact ) );
		(*nop)->num_poss_preconds = 0;
		tmp_ft = &((*nop)->poss_preconds[0]);
		tmp_ft->predicate = w->fact->predicate;
		a = garity[tmp_ft->predicate];
		for ( j = 0; j < a; j++ ) {
			tmp_ft->args[j] = w->fact->args[j];
		}
		(*nop)->poss_precond_annotation_id[0] = ++gnum_possible_annotations;
		(*nop)->num_poss_preconds = 1;
		break;
	case TRU:
		(*nop)->poss_preconds = NULL;
		(*nop)->num_poss_preconds = 0;
		break;
	default:
...
void make_normal_possible_effects(NormOperator **nop, Operator *op) {
	Effect *e;
	Fact *tmp_ft;
	Literal *l;
	int ma, md, j;
	...
	if (e->next) {
		printf("Invalid two possible effects. File: %s, line %d\n\n",__FILE__, __LINE__);
		exit(1);
	}
	...
	(*nop)->poss_add_annotation_id = (int*) calloc(ma, sizeof(int));
	(*nop)->poss_adds = ( Fact * ) calloc( ma, sizeof( Fact ) );
	(*nop)->num_poss_adds = 0;
	(*nop)->poss_del_annotation_id = (int*) calloc(md, sizeof(int));
	(*nop)->poss_dels = ( Fact * ) calloc( md, sizeof( Fact ) );
	(*nop)->num_poss_dels = 0;

	for (l = e->effects; l; l = l->next) {
		if (l->negated) {
			(*nop)->poss_del_annotation_id[(*nop)->num_poss_dels] = ++gnum_possible_annotations;
			tmp_ft = &((*nop)->poss_dels[(*nop)->num_poss_dels++]);
		}
		else {
			(*nop)->poss_add_annotation_id[(*nop)->num_poss_adds] = ++gnum_possible_annotations;
			tmp_ft = &((*nop)->poss_adds[(*nop)->num_poss_adds++]);
		}
